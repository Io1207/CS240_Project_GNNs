...
class SecondOrderRandomWalker:
    ...
    def node2vec_walk(self, start_node):
        #shows random walk from starting to ending node
        G = self.G
    ...
    def get_alias_edge(self, src, dst):
        #Get the alias edge setup lists for a given edge (with the newly setup ids)
        G = self.G
        p = self.p
        q = self.q
...
def alias_draw(J, q):
    # sampling from non uniform discrete distribution to sample aliases
    K = len(J)
